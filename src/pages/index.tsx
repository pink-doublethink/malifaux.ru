...
const Home = ({ posts }) => {
  ...
  const { name, image, designation, bio, bio1 } = config.profile;
    ...
    <Base>
          ...
          <div className="row">
            <div className="mx-auto text-center lg:col-8">
              <ImageFallback
                ...
                alt={name}
              />
              {markdownify(name, 'h1', 'mt-12 text-6xl lg:text-8xl font-semibold')}
              {markdownify(designation, 'p', 'mt-6 text-primary text-xl')}
              {markdownify(bio, 'p', 'mt-4 leading-9 text-xl')}
              {/* {markdownify(bio1, "p", "mt-4 leading-9 text-xl")} */}
      ...
      <div className="pt-4">
        <div className="container">
          ...
          <div className="mt-12">
            <Pagination totalPages={Math.ceil(posts.length / pagination)} currentPage={1} />
          </div>
        </div>
      </div>
    </Base>
